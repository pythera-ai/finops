...
def main(market_data: Dict[str, Any], top_count: int = 3) -> Dict[str, Any]:
    ...
    Args:
        ...
        top_count: Số lượng chỉ số top cần lấy

    Returns:
        Thông tin chỉ số top tăng/giảm
    """
    ...
    try:
        ...
        # Tạo danh sách tất cả chỉ số với thông tin chi tiết
        all_indices = []
        ...
        print(f"Successfully extracted top {top_count} performing indices")
        print(f"Top gainers: {[idx['name'] for idx in top_gaining_indices]}")
        print(f"Top losers: {[idx['name'] for idx in top_losing_indices]}")

        return result

    except Exception as e:
        print(f"Error extracting indices performance: {str(e)}")
        return {
            "success": False,
            "error": str(e),
            "top_gaining_indices": [],
