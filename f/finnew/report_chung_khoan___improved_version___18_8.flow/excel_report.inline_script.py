...
def create_enhanced_excel_report(data: Dict[str, Any]) -> str:
    ...
    # Khối ngoại with rich text
    kn_data = data.get("khoi_ngoai", {})
    if kn_data:
        vol = kn_data.get("vol", 0)
        net_value = kn_data.get("net_value", 0)

        vol_color_key = get_color_based_on_percentage(str(vol))
        net_color_key = get_color_based_on_percentage(str(net_value))

        vol_text = f"↑{vol:.2f}" if vol >= 0 else f"↓{abs(vol):.2f}"
        net_text = f"↑{net_value:.2f}" if net_value >= 0 else f"↓{abs(net_value):.2f}"

        khoi_ngoai_rich_text = [
            TextBlock(
                InlineFont(
                    rFont="Times New Roman",
                    sz=11,
                    b=True,
                    color=colors[net_color_key],  # base on net_text
                ),
                "Khối ngoại: ",
            ),
            TextBlock(
                InlineFont(
                    rFont="Times New Roman", sz=11, b=True, color=colors[vol_color_key]
                ),
                f"{vol_text} triệu cp ",
            ),
            TextBlock(
                InlineFont(
                    rFont="Times New Roman", sz=11, b=True, color=colors[net_color_key]
                ),
                f"{net_text} tỷ đồng",
            ),
        ]
        sections.append((CellRichText(khoi_ngoai_rich_text), None))
